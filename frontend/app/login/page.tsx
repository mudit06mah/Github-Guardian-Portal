...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.auth.loginWithPassword(formData.email, formData.password)
      ...
      const statusResponse = await fetch(`http://localhost:8000/api/auth/github/app-status/${response.user.id}`)
      if (statusResponse.ok) {
        const statusData = await statusResponse.json()
        if (statusData.app_installed) {
          router.push("/")
        } else {
          toast({
